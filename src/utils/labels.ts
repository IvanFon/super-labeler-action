...
export async function sync(this: Utils, config: Runners["labels"]) {
	...
	const curLabels: Labels = await this.api.labels.get().catch((err) => {
		throw log(LoggingLevels.error, `Error thrown while getting labels: ` + err)
	})
	...
	for (const configLabel of Object.values(config)) {
		...
		if (label) {
			if (
				...
				log(
					LoggingLevels.debug,
					`current name: ${label.name.toString()} | new: ${configLabel.name.toString()}`
				)
				await this.api.labels.update(label.name, configLabel).catch((err) => {
					log(LoggingLevels.error, `Error thrown while updating label: ` + err)
				})
			} else {
				log(
					LoggingLevels.info,
